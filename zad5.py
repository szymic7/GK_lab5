...
def calculateNormalVectors():
    ...
    # Obluga biegunow
    for j in range(0, N + 1):
        matrixVectors[0][j] = [0.0, -1.0, 0.0]
        matrixVectors[N][j] = [0.0, 1.0, 0.0]




def drawNormalVectors(tab):
    for i in range (0, N):
        for j in range (0, N):
            glBegin(GL_LINES)
            glVertex3f(tab[i][j][0], tab[i][j][1], tab[i][j][2])
            glVertex3f(tab[i][j][0] + matrixVectors[i][j][0], tab[i][j][1] + matrixVectors[i][j][1], tab[i][j][2] + matrixVectors[i][j][2])
            glEnd()


def render(time):
    global theta, phi, showNormalVectors
    ...
    if showNormalVectors == 1:
        drawNormalVectors(tab)
